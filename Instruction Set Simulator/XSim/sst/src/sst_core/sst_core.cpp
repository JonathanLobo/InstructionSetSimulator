...
std::vector<std::string> instruction_trace;
...
void core::execute(std::string instruction) {
	...
	int instruction_int = (int)strtol(instruction.c_str(), NULL, 2);
	char instruction_hex[16];
	sprintf(instruction_hex, "%04x", instruction_int);
	// std::cout << instruction_hex <<std::endl;
	std::string hexString(instruction_hex);
	...
	if (opcode == "00000") {
		...
		char address_hex[16];
		sprintf(address_hex, "%04x", address);
		std::string hexAddress(address_hex);
		hexString = hexString + " " + hexAddress;
		...
		//j
		std::string pc_bits = std::bitset<16>(pc).to_string().substr(0, 4);
	...
	if (outputThis == 1) {
		instruction_trace.push_back(hexString);
	}
	...
	if (!busy) {
		...
		if (state == 0) {
			...
			if ((pc/2) < program.size() && !halt) {
				...
				std::ofstream outFile("instruction_trace.txt");
    		for (const auto &e : instruction_trace) outFile << e << "\n";
